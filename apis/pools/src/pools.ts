...
export const getPoolsObjectNameByDate = (chainId: string | number, date?: Date | number) =>
  `extended_pools_${chainId}_${date ? formatToDate(date) : 'latest'}`
...
export const getPoolsTvlObjectNameByDate = (chainId: string | number, date?: Date | number) =>
  `extended_pools_tvl_${chainId}_${date ? formatToDate(date) : 'latest'}`
...
export function poolsRoute(router: Router) {
  ...
  createRoute('/v0/extended-pools/:chainId', getPoolsObjectNameByDate)
  createRoute('/v0/extended-pools-tvl/:chainId', getPoolsTvlObjectNameByDate)
}
